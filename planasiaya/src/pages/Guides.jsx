...
//   // 🔄 Alternar favoritos (ya activa/desactiva ciudades en UserContext)
//   const toggleFavorite = (destination) => {
//     if (!user) {
//       alert("Inicia sesión para guardar favoritos ❤️");
//       return;
//     } const slug = normalizeCity(destination);
...
//                     whileTap={{ scale: 0.8 }}
//                   >
//                    {profile?.favorites?.includes(normalizeCity(guide.name)) ? "❤️" : "🤍"}
//                   </motion.button>
      ...
      >
        ...
        {guidesData.map((country, index) => (
          ...
          >
            ...
            >
              {country.destinations.map((guide) => {
                const slug = normalizeCity(guide.name);
                return (
                  <motion.div
                    key={guide.name}
                    className="relative group block bg-white dark:bg-darkCard rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300"
                  ...
                  >
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.8 }}
                    >
                      {profile?.favorites?.includes(slug) ? "❤️" : "🤍"}
                    </motion.button>
